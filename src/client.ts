...
export class UploadxClient {
  ...
  constructor(config: UploadConfig = {}) {
    ...
    axiosRetry(this.client, {
      ...
      retryCondition: (error) => {
        const isNetworkError = axiosRetry.isNetworkError(error) || (error as AxiosError & { code?: string }).code === 'ECONNRESET';
        return (
          isNetworkError || axiosRetry.isRetryableError(error) || error.response?.status === 429 || (error.response?.status || 0) >= 500
        );
      },
      ...config.retryConfig,
    });
   ...
   */
  async updateUpload(url: string, metadata: Partial<UploadMetadata>, signal?: AbortSignal): Promise<void> {
    try {
   ...
   */
  async createUpload(endpoint: string, metadata: UploadMetadata, signal?: AbortSignal): Promise<{ url: string; uploadedBytes?: number }> {
    try {
  ...
  async fileUpload(
    ...
    try {
      const session = await this.createFileUpload(filePath, metadata, endpoint, signal);
      const sessionUrl = session.url;
      const start = session.uploadedBytes || 0;
      const totalSize = metadata.size;

      await this.uploadFileInChunks(sessionUrl, filePath, start, totalSize, onProgress, signal);
    } catch (error) {
  ...
  async upload(
    ...
    try {
      const { url, uploadedBytes } = await this.createUpload(endpoint, metadata, signal);
  ...
  async resumeFileUpload(
    ...
    try {
      ...
      await this.uploadFileInChunks(url, filePath, start, totalSize, onProgress, signal);
    } catch (error) {
      // Don't throw if the operation was aborted
      if (error instanceof AbortError) {
        return;
   ...
   */
  async getUploadStatus(url: string, metadata?: Partial<UploadMetadata>, signal?: AbortSignal): Promise<{ uploadedBytes: number }> {
    const headers = {
  ...
  private async uploadDataInChunks(
    ...
    if (this.isBlob(data)) {
      await this.uploadBlobInChunks(url, data, start, totalSize, onProgress, signal);
    } else if (this.isStream(data)) {
      await this.uploadStreamInChunks(url, data, start, totalSize, onProgress, signal);
    } else {
      await this.uploadBufferInChunks(url, data, start, totalSize, onProgress, signal);
    }
  }
  ...
  private async uploadFileInChunks(
    ...
    while (position < totalSize) {
      ...
      const result = await this.uploadChunk(url, fileChunk, position, end, totalSize, onProgress, signal);
  ...
  private async uploadBlobInChunks(
    ...
    while (position < totalSize) {
      ...
      await this.uploadChunk(url, chunk, position, end, totalSize, onProgress, signal);
      position = end;
    }
  }
  ...
  private handleError(error: unknown, context: string): Error {
    ...
    if (axios.isAxiosError(error)) {
      return new Error(`${context}: ${error.message} ${error.response?.data?.error?.message || error.cause} `);
    }

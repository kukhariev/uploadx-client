...
async function main() {
  const endpoint =
    process.env.UPLOADX_ENDPOINT || 'http://localhost:3002/files';
  ...
  if (!filePath) {
    console.error(
      'Please provide a file path as an argument or set the UPLOADX_EXAMPLE_FILE_PATH environment variable.',
    );
    process.exit(1);
  }
